...
enyo.kind({
    ...
    statics: (function() {
        ...
        function binb_sha256(key, len) {
            ...
            /* append padding */
            bkey[len >> 5] |= 0x80 << (24 - len % 32);
            bkey[((len + 64 >> 9) << 4) + 15] = len;

            for (i = 0; i < bkey.length; i += 16) {
                a = HASH[0];
                b = HASH[1];
                c = HASH[2];
                d = HASH[3];
        ...
        return {
            hash: function(inString) {
                ...
                var endianHash = binb_sha256(inString);
            ...
            hmac: function(inString, inData) {
                ...
                var stringHash = rstr_hmac_sha256(inString, inData);
                return new enigma.Hash(stringHash);
            }
        };
    })()
